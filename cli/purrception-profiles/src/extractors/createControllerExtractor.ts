...
const DEFAULT_MATCHER = (ctx: ModuleExtractContext) => {
	const match = ctx.moduleMeta.relative.match(/^([^/]+)\/controllers\//);
	if (match) {
		const category = match[1];
		const name = path.basename(ctx.moduleMeta.path);
		const meta = { type: 'controller', name, category };
...
export function createControllerExtractor(options: Options): ModuleEntityExtractor {
	return async ctx => {
